...
func (ki *KeyIterator) ExecuteKeyQuery(prefix string, req pagination.PaginationRequest) ([]string, pagination.PaginationResponse, error) {
	...
	fmt.Printf("[KeyIterator] Query: prefix=%q, Before=%q, After=%q, Anchor=%q, Limit=%d, SortBy:%s, OrderBy:%s\n",
	...
	if err != nil {
		fmt.Printf("[KeyIterator] Failed to create iterator: %v\n", err)
	...
	default:
		...
		fmt.Printf("[KeyIterator] Initial load: Limit=%d\n", req.Limit)
	...
	if len(sortedKeys) > 0 {
		fmt.Printf("[KeyIterator] Result anchors: StartAnchor=%q, EndAnchor=%q, Count=%d, HasBefore=%v, HasAfter=%v\n",
			response.StartAnchor, response.EndAnchor, response.Count, response.HasBefore, response.HasAfter)
	} else {
		fmt.Printf("[KeyIterator] No keys returned from query.\n")
	}
