...
echo "=== Create Messages Benchmark ==="
echo "This benchmark creates messages under a pre-existing thread"
echo ""
...
mkdir -p "$ART_DIR"
TEST_ID="create-messages-$(date +%Y%m%d%H%M%S)"
OUT_JSON="$ART_DIR/${TEST_ID}.json"

echo "Running create-messages benchmark..."
k6 run --out json=$OUT_JSON service/tests/benching/k6/create-message.js
